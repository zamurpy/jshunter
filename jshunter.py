...
# thread-safe append writer, flushes on every write. json format = jsonl
class LiveWriter:
...
class JSHunter:
    def __init__(self, base_url, depth=1, threads=10, delay=0.0, timeout=10,
                 ...
                 bug_mode=False, verbose=True, writer=None, resume_seen=None):
        ...
        for level in range(1, self.depth + 1):
            ...
            with ThreadPoolExecutor(max_workers=self.threads) as ex:
                ...
                for fut in as_completed(futures):
                    ...
                    js_files, page_links = self.extract_from_html(page, r.text)
                    for js in js_files:
                        self.add_js(js)
